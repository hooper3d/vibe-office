...
- Local trusted agent readiness UI state now lives in `src/services/agentReadinessController.ts`, keeping readiness issue/status storage, refresh, replacement, and deletion cleanup out of `App.tsx`.
- Configured agent sync now lives in `src/services/agentStorage.ts`, pairing local trusted registry upserts with browser-safe metadata persistence while keeping provider credentials out of localStorage.
...
- Trackable task output normalization now lives in `src/services/projectTaskOutputItems.ts`, so `ProjectTasks.tsx` renders one unified task-output list instead of separately mapping run records and standalone task records.
- Artifact content parsing now lives in `src/services/projectArtifactContent.ts`, while artifact list/detail/preview rendering lives in `src/components/ProjectArtifactViewer.tsx`; `ProjectArtifacts.tsx` now keeps selection and action orchestration focused.
...
- Task lifecycle busy-action ids now live in `src/services/taskLifecycleState.ts`, keeping refresh/retry/cancel status keys shared by App and ProjectTasks.
...
- Browser preview links are now assigned into grouped project outputs as a Preview type owned by the selected agent that opened the preview, so the Output Area lookup path is group first, then Tasks / Artifacts / Preview.
...
- Main workspace layout, split-pane chrome, Conversation Area composition, and Output Area composition now live in `src/components/MainWorkspace.tsx`; `App.tsx` wires state/controllers and no longer owns the central/right panel JSX directly.
...
- Add/Edit Agent provider settings now live in `src/components/AgentProviderSettings.tsx`, and reusable setup labels/selectors live in `src/components/AgentSetupFieldLabel.tsx` and `src/components/AgentProfileSelectors.tsx`, keeping `SetupWizard.tsx` focused on modal composition.
...
- Free Chat history activation and new-conversation control now live in `src/services/freeChatController.ts`, keeping per-agent history selection, empty-chat reuse, and free-chat chrome switching out of `App.tsx`.
...
- App synchronization control now lives in `src/services/appSyncController.ts`, keeping runtime-store sync, configured-agent sync, local trusted readiness refresh, UI/workspace persistence, theme persistence, and workspace-attachment reset out of `App.tsx`.
- Pending request recovery control now lives in `src/services/pendingRecoveryController.ts`, keeping interrupted Free Chat / Project Chat recovery after refresh or app interruption out of `App.tsx`.
- App selection control now lives in `src/services/appSelectionController.ts`, keeping selected-agent fallback, missing-project recovery, Free Chat conversation-mode normalization, Task Room participant reset, and participant toggles out of `App.tsx`.
...
- App agent view state now lives in `src/services/appAgentViewState.ts`, keeping selected agent, Chief lookup, readiness issue map, responding agent ids, and Task Room participant derivation out of `App.tsx`.
