...
export function TaskList({
  ...
  const executableSelectedCount = tasks.filter(
    (task) => task.selected && task.planStatus !== "completed" && task.planStatus !== "executing" && task.planStatus !== "reviewing"
  ).length;
      ...
      {!embedded ? (
        <div className="mb-4 flex shrink-0 items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <h2 className="text-base font-semibold text-slate-100">Task Desk</h2>
          </div>
          <span className="soft-pill bg-slate-500/10 px-2 py-0.5 text-xs text-slate-300">{tasks.length} tasks</span>
        </div>
      ) : null}
      ...
      {selectable ? (
        <div className="mb-3 flex shrink-0 items-center justify-between gap-3 rounded-lg border border-slate-800 bg-slate-950/20 px-3 py-2">
          <span className="text-xs font-medium text-slate-400">
            {selectedCount} selected, {executableSelectedCount} executable
          </span>
          <button
            type="button"
          ...
          >
            ...
            <span>{executionDisabledReason ? "Runner disabled" : "Execute selected"}</span>
          </button>
        </div>
      ) : null}
      ...
      <div className="scrollbar-thin min-h-0 min-w-0 flex-1 space-y-2 overflow-auto pr-1">
        {!tasks.length ? (
          <div className="flex h-full min-h-[180px] flex-col items-center justify-center rounded-lg border border-dashed border-slate-800 bg-slate-950/10 px-6 text-center">
            <p className="text-sm font-semibold text-slate-300">No planned tasks yet</p>
            <p className="mt-2 max-w-[260px] text-xs leading-5 text-slate-500">
              Chat with Chief or generate a plan workflow first. Selectable execution tasks will appear here.
            </p>
          </div>
        ) : null}
        {tasks.map((task) => {
          ...
          return (
            ...
            >
                ...
                <div className="flex min-w-0 items-center gap-2">
                  {selectable ? (
                    <input
                      ...
                      disabled={running || task.planStatus === "executing" || task.planStatus === "reviewing" || task.planStatus === "completed"}
              ...
              {task.acceptance?.length ? <p className="mt-2 truncate text-[11px] text-slate-500">Acceptance: {task.acceptance[0]}</p> : null}
            </div>
          );
        })}
      </div>
